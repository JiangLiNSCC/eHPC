...
def ssh(machine_name, hostname):
    # Do a single ping with timeout 2 seconds
    (output, err, retcode) = run_command('ssh %s hostname' % hostname)
    if retcode == 0:
...
def get_status(machine_name=None):
    ...
    if machine_name == None:
        return [ ssh(s['NAME'], s['HOSTNAME']) for s in conf['SYSTEMS'] ] 
        ...
        return ssh(machine_name, hostname)
