...
from account.models import Account , AccountActive , UserAccount
import re
import logging
...
def get_user_info(user_name, uid):
    ...
    try:
        ...
        del user_dict["password"] , user_dict["groups"] , user_dict["user_permissions"]
...
def bind_account( requeset , account , password = None , keyFile = None ):
    # check account by ldap or ssh
    user = request.user
    ...
    if password :
        ...
        #checkSSH
        pass
    acc , acc_c =  Account.objects.get_or_create( account = account )
    UserAccount.objects.create()
    # if active
    AccountActive.objects.create(user=user, account=acc)
    return "Bind Account OK"
